...
func TestMainExec(t *testing.T) {
	...
	tests := []testStruct{
		...
		{
			name:                         "GET /thing/search with existing keyword and validated false should return empty",
			wantStatusCode:               http.StatusOK,
			contentType:                  MIMEHtmlCharsetUTF8,
			wantBody:                     "[]",
			paramKeyValues:               make(map[string]string),
		...
		{
			name:                         "GET /thing/search with non-existing keyword should return ok and empty array",
			wantStatusCode:               http.StatusOK,
			contentType:                  MIMEHtmlCharsetUTF8,
			wantBody:                     "[]", // should return an empty array
			paramKeyValues:               make(map[string]string),
