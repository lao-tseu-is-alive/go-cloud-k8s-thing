...
const getSessionKey = (appName: string, key: string): string => `${appName}${key}`
...
const getSessionItem = (appName: string, key: string): string | null => {
  log.t(`session get called with key:${key}, appName:${getSessionKey(appName, key)}`)
...
export const parseJwt = (token: string): JwtPayload => {
  log.t(`# entering parseJwt... token:${token} `)
  try {
...
export const getRemoteJwtToken = async (
  appName: string,
  ...
  log.t(`# entering getRemoteJwtToken...app:${appName}, loginUrl:${loginUrl}, username: ${username}`)
  ...
  try {
    ...
    log.w("getToken() axios.post Success! response:", response.data)
    ...
    setSessionItem(appName, SESSION_KEYS.USER_ID, String(jwtValues.User.user_id))
    setSessionItem(appName, SESSION_KEYS.USER_EXTERNAL_ID, jwtValues.User.external_id)
    ...
    setSessionItem(appName, SESSION_KEYS.IS_ADMIN, String(jwtValues.User.is_admin))
    setSessionItem(appName, SESSION_KEYS.DATE_EXPIRATION, String(jwtValues.exp))
...
export const getUserProfile = (appName: string): UserProfile | null => {
  ...
  return {
    ...
    userExternalId: getSessionItem(appName, SESSION_KEYS.USER_EXTERNAL_ID) ?? "",
...
export const logoutAndResetToken = async (appName: string, baseServerUrl: string): Promise<void> => {
...
export const getLocalJwtTokenAuth = (appName: string): string => getUserProfile(appName)?.jwtToken ?? ""
export const getDateExpiration = (appName: string): number => getUserProfile(appName)?.dateExpiration ?? 0
export const getUserEmail = (appName: string): string => getUserProfile(appName)?.email ?? ""
export const getUserId = (appName: string): number => getUserProfile(appName)?.userId ?? 0
export const getUserName = (appName: string): string => getUserProfile(appName)?.name ?? ""
export const getUserLogin = (appName: string): string => getUserProfile(appName)?.username ?? ""
export const getUserIsAdmin = (appName: string): boolean => getUserProfile(appName)?.isAdmin ?? false
export const getSessionId = (appName: string): string => getUserProfile(appName)?.sessionUuid ?? ""
...
export const getUserFirstGroups = (appName: string): number | null => getUserGroupsArray(appName)?.[0] ?? null
