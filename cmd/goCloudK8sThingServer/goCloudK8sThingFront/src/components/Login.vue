          ...
          <v-card class="elevation-12">
            ...
            <v-card-text>
              <v-form ref="loginform" v-model="validLoginForm" lazy-validation>
                <v-text-field id="username" name="username" ref="username"
                              v-model="username" required autocomplete="username"
                              prepend-icon="person" :cleareable="true"
                              :rules="[() => !!username ||
                            'Veuillez saisir votre login, il est obligatoire']"
                              v-on:keyup.enter="onEnterKey"
                              label="Entrez votre utilisateur" type="text">
                </v-text-field>
                <!-- add this to get chrome autocomplete
                browser-autocomplete="current-password"-->
                <v-text-field id="password" name="password" ref="password"
                              v-model="password" required
                              autocomplete="current-password"
                              prepend-icon="lock"
                              :rules="[() => !!password ||
                            'Veuillez saisir votre mot de passe, il est obligatoire']"
                ...
                <v-text-field id="password_hash" name="password_hash" ref="password_hash"
                              v-show="sha256Visible"
                              v-model="password_hash"
                              prepend-icon="lock"
                              label="sha256" type="text">
                </v-text-field>
              </v-form>
            </v-card-text>
            <v-card-actions>
