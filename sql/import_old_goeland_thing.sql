...
FROM goeland.goeland_thing_gc T WHERE idcreator is null
GROUP BY idtypething;

SELECT COUNT(*),t.idtypething,
       (SELECT name FROM goeland.goeland_type_thing tt WHERE tt.idtypething = t.idtypething),
       t.name, min(t.idthing), max(t.idthing)
...
select count(*) FROM goeland.goeland_thing_gc where isactive=true;
select count(*) FROM  go_thing.thing;
